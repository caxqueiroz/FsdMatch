...
A Go CLI + MCP server that validates a Functional Specification Document against a Java/Spring Boot codebase. Single static binary, SQLite + vec0 storage, Bedrock Codex for atomization and judgment. See `SPEC.md` §3 for the architecture.
...
6. **Evidence is mandatory.** Any match verdict from Codex that does not include `file:start_line-end_line` evidence is downgraded to `unrelated`. No exceptions.
